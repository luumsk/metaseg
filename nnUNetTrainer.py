    ...
    def do_split(self):
        ...
        if self.fold == "all":
            ...
            if not isfile(splits_file):
                ...
                # Split folds by patients, make sure all timestamps of one patient stay in the same fold
                # Extract IDs from dataset keys
                all_keys = list(dataset.keys())
                ids = ['-'.join(key.split('-')[:3]) for key in all_keys]  # Extract the first three segments for the ID
                
                # Group keys by their ID
                id_to_keys = {}
                for key, id in zip(all_keys, ids):
                    if id not in id_to_keys:
                        id_to_keys[id] = []
                    id_to_keys[id].append(key)
                
                # Sort IDs
                sorted_ids = np.sort(list(id_to_keys.keys()))
                
                # Create KFold splits by IDs
                kfold = KFold(n_splits=5, shuffle=True, random_state=12345)
                splits = []
                
                for i, (train_idx, test_idx) in enumerate(kfold.split(sorted_ids)):
                    train_ids = sorted_ids[train_idx]
                    test_ids = sorted_ids[test_idx]
                    
                    train_keys = [key for id in train_ids for key in id_to_keys[id]]
                    test_keys = [key for id in test_ids for key in id_to_keys[id]]
